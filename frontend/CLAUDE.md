...
- E2E：website.spec.ts 覆盖官网公开页面与路由守卫（无需后端）；workbench-flow.spec.ts 覆盖注册、创建简历、编辑器、分享和 Markdown 导出（需本地后端）。
- 真实头像 AI、简历导入和富文本编辑属于后续迭代。AI 写作按日配额（游客 3/天、登录 30/天）已启用。
...
- 当前基线：15 个测试文件、44 个单元/组件测试通过；E2E 覆盖官网 + 工作台核心流程，工作台流程需本地后端。
