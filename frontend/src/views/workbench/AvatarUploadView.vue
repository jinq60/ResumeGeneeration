...
async function handleOptimize() {
  ...
  optimizeTask.value = null
  try {
    // 携带 resumeId 时，后端优化成功后会回填简历 profile.avatarUrl
    const requestBody: OptimizeAvatarRequest = { ...optimizeForm.value }
    if (resumeId) {
      requestBody.resumeId = resumeId
    }
    const response = await avatarApi.optimize(requestBody)
...
function handleApplyToResume() {
  ...
  if (!resumeId) {
    ElMessage.info('未关联简历，无法应用。请从简历编辑页的个人信息模块进入本页后重新优化。')
    return
  }
